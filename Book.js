...
app.get("/Form", (req, res) =>
  res.sendFile(__dirname + "/EmployeeAdding.html")
);

app.get("/TableOfRecords", (req, res) =>
  ...
  pool
    ...
    .then(() => {
      pool.request().query(query, (err, result) => {
        ...
        else res.send("Employee inserted successfully");
      });
    })
    .catch((err) => {
      console.error(err);
  ...
  pool
    ...
    .then(() => {
      pool.request().query("SeleCT * FROM EMPLOYEE", (err, results) => {
        ...
        else res.send(results.recordset);
      });
    })
    .catch((err) => {
      if (err) console.log(err);
...
app.get("/:id", (req, res) => {
  ...
  pool
    ...
    .then(() => {
      pool
        .request()
        .query(`select * from employee where empId = ${id}`, (err, result) => {
          if (err) console.error(err);
...
app.delete("/:id", (req, res) => {
  ...
  pool
    ...
    .then(() => {
      pool
        .request()
        .query(`delete from employee where empId = ${id}`, (err, result) => {
          if (err) console.error(err);
          else res.send("record deleted successfully");
        });
    })
    .catch((err) => {
      if (err) console.log(err);
